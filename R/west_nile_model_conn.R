#' west_nile_model_conn() habia una vez ...
#'
...
#' tModelDisease <- base::paste(tMetaData,"_west_nile",sep="") 
#' q         <- base::paste("select r_table_exists('", tModelDisease,"')", sep="")
#' res       <- RPostgres::dbSendQuery(conn, q)
#' exists    <- RPostgres::dbFetch(res)
#' RPostgres::dbClearResult(res)
#' if( as.integer( exists) ){ print("Exists!") }else{ west_nile_model_conn(tMetaData, disease, conn, degree) }
#' RPostgres::dbDisconnect(conn)
#' @export
west_nile_model_conn <- function(tMetaData, disease, conn, degree){
  library(RcppOctave)
