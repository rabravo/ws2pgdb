...
all_coor_ws <- function( ghcnd, geoid, type) {
  ...
  tryCatch(
    {
      ws       <- NULL
      stations <- NULL
      ws       <- rnoaa::ncdc_stations(datasetid=ghcnd, datatypeid=type, locationid=FIPS, limit=1000, token=file$token)
      stations <- ws$data
    },error = function (err_msg){
        message(base::paste("msg: ", err_msg, sep=""))
        return(stations)
  ...
  # Handle special case for FIPS 48113 where stations have identical data
  if(geoid == '48113'){
      switch(type,
